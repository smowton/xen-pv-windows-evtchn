...
struct gntmem_handle* gntmem_open() {
	...
	wchar_t* gntmem_if_path = get_xen_interface_path();
	HANDLE h;
	struct gntmem_handle* new_handle;

	if(gntmem_if_path) {
		h = CreateFile(gntmem_if_path, FILE_GENERIC_READ|FILE_GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED | FILE_ATTRIBUTE_NORMAL, NULL);
		free(gntmem_if_path);
	}
	else {
		return NULL;
