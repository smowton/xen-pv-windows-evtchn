...
get_xen_interface_path()
{
  ...
  path = (wchar_t*)malloc((wcslen(sdidd->DevicePath) + 1) * sizeof(wchar_t));
  wcscpy(path, sdidd->DevicePath);
  free(sdidd);
...
HANDLE xc_evtchn_open(void) {
	...
	wchar_t* path = get_xen_interface_path();
	if(path) {
		HANDLE h = CreateFile(path, FILE_GENERIC_READ|FILE_GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED, NULL);
		free(path);
		return h;
	}
	else {
...
xc_evtchn_bind_unbound_port(HANDLE xce_handle, int domid) {
	...
	ioctl_evtchn_bind_unbound_port params;
...
xc_evtchn_bind_interdomain(HANDLE xce_handle, int domid, evtchn_port_t remote_port) {
	...
	ioctl_evtchn_bind_interdomain params;
	unsigned int port_out;
	DWORD bytes_written;
	OVERLAPPED ol;
	memset(&ol, 0, sizeof(ol));
