...
get_xen_interface_path()
{
  ...
  path = malloc((wcslen(sdidd->DevicePath) + 1) * sizeof(wchar_t));
...
struct xs_handle *xs_domain_open(void) {
	...
	if (path == NULL) {
		fprintf(stderr, "Xenstore error: Failed to open Xen kernel device; is XenPCI loaded?\n");
		return NULL;
	}

	handle = CreateFile(path, FILE_GENERIC_READ|FILE_GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED, NULL);
	if(handle == INVALID_HANDLE_VALUE) {
		fprintf(stderr, "Xenstore error: Failed to open kernel device %s, due to error %ul\n",
				path, GetLastError());
		free(path);
		return NULL;
	}
